...
if (!createRecipeResponse.ok) {
  const errorBody = await createRecipeResponse.json().catch(() => createRecipeResponse.text());
  console.error("❌ Strapi error status:", createRecipeResponse.status);
  console.error("❌ Strapi error body:", JSON.stringify(errorBody, null, 2));
  console.error("❌ Data that failed:", JSON.stringify(strapiRecipeData.data, null, 2));
  throw new Error(`Strapi save failed: ${createRecipeResponse.status}`);
}
